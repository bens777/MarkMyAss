...
## Hosted GhostMark (https://moseisley.sh/ghostmark)
...
- Uploaded files and pasted text are sent to, and temporarily processed
  on, the GhostMark server running at moseisley.sh.
...
- The cleaned file is deleted **immediately** after you download it. If
  you never download it, the whole session (original + cleaned copy) is
  deleted automatically after a short timeout -- 10-15 minutes maximum
  (see `GHOSTMARK_SESSION_TTL_MINUTES` in `docker-compose.prod.yml` for
...
of [ExifTool](https://exiftool.org/) to independently cross-check a
cleaned file. This happens entirely within the same process/server that
already has the file -- ExifTool is not a network service, and no data is
sent anywhere by using it.
