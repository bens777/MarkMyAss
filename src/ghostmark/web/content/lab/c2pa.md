...
**MarkMyAss's C2PA support is explicitly labeled Partial, and stays that
...
MarkMyAss's own detector scans for the JUMBF container structure itself
...
container MarkMyAss scans for -- it is not an audited guarantee against
...
- **Cryptographic signature or trust-chain validation.** Neither
  MarkMyAss's own detector nor its use of `c2patool` performs signature
...
- Manifests embedded via non-standard containers MarkMyAss doesn't scan
  for.
- Whether removing the container also removed every trace referenced
  ...
  MarkMyAss's cleaner targets the JUMBF container specifically.
...
anything). MarkMyAss uses c2patool only to confirm presence/absence of a
...
c2patool is optional and licensed Apache-2.0/MIT; MarkMyAss never vendors
it, only shells out to a separately-installed binary (see
`THIRD_PARTY_LICENSES.md`). If it isn't installed, MarkMyAss says so
