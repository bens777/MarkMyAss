...
### What GhostMark actually sees, removes, and cannot cryptographically validate
...
or other media file. This page explains exactly what GhostMark can and
can't do with one.
...
GhostMark scans for the **structural presence** of a JUMBF container —
the byte-level markers indicating a C2PA manifest is embedded. This is a
heuristic structural scan, not a manifest parser: GhostMark doesn't read
the manifest's claims, verify its signature, or check a trust chain.

## What GhostMark can remove

If a JUMBF container is present, GhostMark can strip it from JPEG and
...
not mean GhostMark broke or forged any cryptography, because it never
...
GhostMark cross-checks its own C2PA detection with the official
...
it is **not a cryptographic trust validator either** in how GhostMark
...
- **Not cryptographic validation.** GhostMark does not verify signatures,
...
- **Absence isn't proof of anything beyond absence.** A file reporting
  "no C2PA container found" tells you GhostMark's structural scan didn't
