...
if ! "$PYTHON_BIN" -c "import sys; sys.exit(0 if sys.version_info >= (3, 11) else 1)"; then
    fail "MarkMyAss needs Python 3.11 or newer. Install it and run this script again."
fi

if [ ! -d ".venv" ]; then
    echo "Setting up MarkMyAss for the first time. This can take a minute..."
    "$PYTHON_BIN" -m venv .venv || fail "Could not create a Python virtual environment."
fi

./.venv/bin/python -m pip install --quiet --upgrade pip
./.venv/bin/python -m pip install --quiet -e . || fail "MarkMyAss could not install its dependencies. See the messages above for details."

echo ""
echo "Starting MarkMyAss at http://127.0.0.1:8765"
